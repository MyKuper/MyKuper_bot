...
DB_FILE = os.getenv('DB_PATH', "bot_data.db")  # Можно задать путь через ENV
...
# ============================================================
# 🗄 БАЗА ДАННЫХ (с диагностикой)
# ============================================================
def init_db():
    logger.info(f"📂 Инициализация БД: {DB_FILE}")
    logger.info(f"📂 Абсолютный путь: {os.path.abspath(DB_FILE)}")
    try:
...
def link_account_db(tg_id, tg_username, player_tag, player_name, clan_tag):
    logger.info(f"💾 Привязка: tg_id={tg_id}, player={player_name}, clan={clan_tag}")
    try:
        conn = sqlite3.connect(DB_FILE)
        c = conn.cursor()
        # Сохраняем пользователя
        c.execute("INSERT OR REPLACE INTO users (tg_id, tg_username) VALUES (?, ?)", 
                  (tg_id, tg_username))
        # Сохраняем аккаунт
        c.execute("""INSERT OR REPLACE INTO linked_accounts 
                     ...
                     VALUES (?, ?, ?, ?)""",
                  (tg_id, player_tag, player_name, clan_tag))
        # УСТАНАВЛИВАЕМ активный клан (REPLACE - всегда перезаписывает!)
        c.execute("INSERT OR REPLACE INTO user_active_clan (tg_id, clan_tag) VALUES (?, ?)", 
                  (tg_id, clan_tag))
        conn.commit()
        
        # ПРОВЕРКА: читаем обратно чтобы убедиться что записалось
        c.execute("SELECT clan_tag FROM user_active_clan WHERE tg_id = ?", (tg_id,))
        check = c.fetchone()
        logger.info(f"✅ Проверка записи: active_clan = {check}")
...
def set_user_active_clan(tg_id, clan_tag):
    logger.info(f"🔄 Смена активного клана: tg_id={tg_id}, clan={clan_tag}")
    try:
        conn = sqlite3.connect(DB_FILE)
        c = conn.cursor()
        c.execute("INSERT OR REPLACE INTO user_active_clan (tg_id, clan_tag) VALUES (?, ?)", 
        ...
        c.execute("SELECT clan_tag FROM user_active_clan WHERE tg_id = ?", (tg_id,))
        check = c.fetchone()
        logger.info(f"✅ Проверка смены клана: {check}")
...
def get_user_active_clan(tg_id) -> Optional[str]:
    try:
        ...
        logger.info(f"🔍 Чтение активного клана: tg_id={tg_id} -> {clan}")
        return clan
    except Exception as e:
        logger.error(f"❌ Ошибка чтения активного клана: {e}")
        return None
...
async def check_user_clan(update) -> Optional[str]:
    ...
    if not clan_tag:
        await send_msg(update, 
            "⚠️ <b>У вас не выбран активный клан!</b>\n\n"
            "1️⃣ Привяжите аккаунт: /link\n"
            "2️⃣ Или укажите клан вручную: /set_clan",
            parse_mode="HTML")
        return None
    return clan_tag
...
def get_main_menu() -> InlineKeyboardMarkup:
    kb = [
        ...
        [InlineKeyboardButton(text="🎯 Указать клан вручную", callback_data="set_clan_menu")],
    ]
    ...
    try:
        ...
        if generate_report:
            try:
                ...
                clan_name = our_clan.name.replace(' ', '_').replace('/', '_')[:30]
                enemy_name = enemy_clan.name.replace(' ', '_').replace('/', '_')[:30]
                ...
                for line in roster_lines:
                    md_content.append(line.replace('<code>', '`').replace('</code>', '`'))
                md_content.append("")
                if lazy_list:
                ...
                if isinstance(update, types.Message):
                    await update.answer_document(FSInputFile(filename), caption=f"📁 Отчет войны")
                else:
                    await update.message.answer_document(FSInputFile(filename), caption=f"📁 Отчет войны")
            except Exception as e:
...
# ============================================================
# 🤖 ОБРАБОТЧИКИ (с проверкой is_bot)
# ============================================================
...
async def cmd_start(message: types.Message):
    ...
    if active_clan:
        text += f"🏰 <b>Активный клан:</b> <code>{active_clan}</code>\n\n"
        text += "Выбери раздел:"
        await message.answer(text, parse_mode="HTML", reply_markup=get_main_menu())
    else:
        text += "⚠️ У вас не выбран активный клан.\n\n"
        text += "1️⃣ Привяжите аккаунт: /link\n"
        text += "2️⃣ Или укажите клан: /set_clan"
...
# ============================================================
# 🎯 УКАЗАТЬ КЛАН ВРУЧНУЮ (новый FSM)
# ============================================================
@dp.message(Command("set_clan"))
@dp.callback_query(F.data == "set_clan_menu")
async def cmd_set_clan(update: types.Message | types.CallbackQuery, state: FSMContext):
    ...
    await msg.answer(
        "🎯 <b>Указать клан вручную</b>\n\n"
        "Отправьте тег клана (например, <code>#2CY00G2VU</code>).\n"
        "Для отмены: /cancel",
        parse_mode="HTML"
    )
...
async def process_clan_tag(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        kb = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="✅ Да, этот клан", callback_data="confirm_clan_yes")],
            [InlineKeyboardButton(text="❌ Нет, отмена", callback_data="confirm_clan_no")]
        ])
        await message.answer(
            f"🏰 Найден клан: <b>{clan.name}</b>\n"
            f"Тег: <code>{tag}</code>\n"
            f"Уровень: <code>{clan.level}</code>\n"
            f"Участников: <code>{clan.member_count}/50</code>\n\n"
            f"Установить как активный?",
            parse_mode="HTML", reply_markup=kb
        )
        await state.set_state(SetClan.waiting_for_clan_confirmation)
    except coc.NotFound:
        await message.answer("❌ Клан не найден. Попробуйте еще раз или /cancel.")
    except Exception as e:
        logger.error(f"Error clan verify: {e}")
...
async def confirm_clan_yes(callback: types.CallbackQuery, state: FSMContext):
    ...
    success = set_user_active_clan(tg_id, data['clan_tag'])
    
    if success:
        await callback.message.edit_text(
            f"✅ Активный клан установлен: <b>{data['clan_name']}</b>\n"
            f"🏰 Тег: <code>{data['clan_tag']}</code>",
            parse_mode="HTML"
        )
    else:
        await callback.message.edit_text("❌ Ошибка сохранения в БД. Попробуйте позже.")
...
async def cmd_link(update: types.Message | types.CallbackQuery, state: FSMContext):
    ...
    await msg.answer(
        ...
        "Отправьте ваш тег игрока (например, <code>#QV2Q9V8L2</code>).\n"
...
async def process_tag(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer("❌ Игрок не найден. /cancel для отмены.")
    except Exception as e:
        logger.error(f"Error verify tag: {e}")
        await message.answer(f"❌ Ошибка: {str(e)[:100]}")
        await state.clear()
...
async def confirm_yes(callback: types.CallbackQuery, state: FSMContext):
    ...
    if not clan_tag:
        await callback.message.edit_text("⚠️ Игрок не в клане. Привязка невозможна.")
        await state.clear()
        return
    
    success = link_account_db(tg_id, tg_username, data['player_tag'], data['player_name'], clan_tag)
    
    if success:
        await callback.message.edit_text(
            f"✅ Аккаунт <b>{data['player_name']}</b> привязан!\n"
            f"🏰 Клан: <code>{clan_tag}</code>\n"
            f"Клан установлен как активный.",
            parse_mode="HTML"
        )
    else:
...
async def cmd_debug(message: types.Message):
    ...
    try:
        ...
        c.execute("SELECT COUNT(*) FROM linked_accounts")
        accounts_count = c.fetchone()[0]
        
        c.execute("SELECT COUNT(*) FROM user_active_clan")
        active_count = c.fetchone()[0]
        
        c.execute("SELECT tg_id, clan_tag FROM user_active_clan WHERE tg_id = ?", (tg_id,))
        my_clan = c.fetchone()
        
        c.execute("SELECT player_name, clan_tag FROM linked_accounts WHERE tg_id = ?", (tg_id,))
        my_accs = c.fetchall()
        
        conn.close()
        
        text = f"🔧 <b>DEBUG INFO</b>\n\n"
        text += f"📂 БД: <code>{DB_FILE}</code>\n"
        text += f"📊 Файл существует: <code>{os.path.exists(DB_FILE)}</code>\n"
        text += f"💾 Размер: <code>{os.path.getsize(DB_FILE) if os.path.exists(DB_FILE) else 0}</code> байт\n\n"
        text += f"👥 Всего пользователей: <code>{users_count}</code>\n"
        text += f"🔗 Всего привязок: <code>{accounts_count}</code>\n"
        text += f"🎯 Всего активных кланов: <code>{active_count}</code>\n\n"
        text += f"👤 <b>Ваши данные:</b>\n"
        text += f"   Активный клан: <code>{my_clan[1] if my_clan else 'НЕТ'}</code>\n"
        text += f"   Привязанные аккаунты: <code>{len(my_accs)}</code>\n"
        for name, ctag in my_accs:
            text += f"   • {name} ({ctag})\n"
...
async def cb_my_accounts(callback: types.CallbackQuery):
    ...
    accounts = get_user_accounts_db(tg_id)
    if not accounts:
        await callback.message.edit_text("🔗 У вас нет привязанных аккаунтов.\n/link", reply_markup=get_back_keyboard("menu_profile"))
        return
...
async def init_coc_client():
    global coc_client
    logger.info("🔄 Инициализация COC клиента...")
    tries = 5
    while tries > 0:
        try:
            ...
            logger.error(f"❌ Ошибка COC (осталось {tries}): {e}")
            tries -= 1
